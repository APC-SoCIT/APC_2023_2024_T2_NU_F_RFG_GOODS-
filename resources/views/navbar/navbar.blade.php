<nav class="bg-orange-400 z-50">
    <div class="flex flex-wrap items-center justify-between mx-auto p-2.5">
        <a href="/" class="pl-5 flex items-center space-x-3 rtl:space-x-reverse pr-8">
        ...
        <div class="relative hidden md:block w-7/12">
            ...
            <form action="{{ route('search') }}" method="GET">
                ...
                <input name="search" type="search" id="search-navbar" class="block w-full p-2.5 ps-10 text-sm text-black border border-orange-200 rounded-lg bg-gray-100 focus:ring-orange-600 focus:border-orange-500" placeholder="Search">
            </form>
        </div>
        ...
        <div class="justify-self-end hidden w-full md:flex md:w-auto md:order-1 ml-auto mr-4" id="navbar-search">
            ...
            <ul class="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 bg-orange-400">
                <li>
                    <a href="/" class="block py-2 px-3 text-white rounded md:bg-transparent md:p-0 md:dark:hover:text-gray-300" aria-current="page">
                ...
                <li>
                    <a href="#" class="block py-2 px-3 text-white rounded md:hover:text-blue-700 md:p-0 md:dark:hover:text-gray-300">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                ...
                <li>
                    <a href="/cart" class="block py-2 px-3 text-white rounded md:hover:bg-transparent md:hover:text-white md:p-0 md:dark:hover:text-gray-300">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
                ...
                @if (Auth::user()!=null)
                    <li>
                        <button id="dropdownNavbarLink" data-dropdown-toggle="dropdownNavbar" class="flex items-center justify-between w-full py-2 px-3 text-gray-900 rounded md:hover:bg-transparent md:border-0 md:hover:text-gray-300 md:p-0 md:w-auto dark:text-white md:dark:hover:text-gray-300 dark:focus:text-white dark:border-orange-600 md:dark:hover:bg-transparent">
                            {{ Auth::user()->first_name }} {{ Auth::user()->last_name }}
                            <svg class="w-2.5 h-2.5 ms-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
                            </svg>
                        </button>
                        <div id="dropdownNavbar" class="z-10 hidden font-normal bg-white divide-y divide-white rounded-lg shadow w-44 dark:bg-orange-400 dark:divide-white">
                            <ul class="py-2 text-sm text-white dark:text-white" aria-labelledby="dropdownLargeButton">
                                <li>
                                    <a href="{{ route('profile.edit') }}" class="block px-4 py-2 md:dark:hover:text-gray-300">Profile</a>
                                </li>
                            </ul>
                            <div class="py-1">
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <a class="block px-4 py-2 text-sm text-white dark:text-white md:dark:hover:text-gray-300">
                ...
                @else
                    ...
                    <li>
                        <a href="{{ route('login') }}" class="block py-2 px-3 text-white rounded md:bg-transparent md:p-0 md:dark:hover:text-gray-300" aria-current="page">Login</a>
                    </li>
                @endif
            </ul>
        </div>
